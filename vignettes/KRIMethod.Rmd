...
The function `Analyze_Poisson()` in `{gsm.core}` utilizes `stats::glm()` to generate an estimate of fitted values as well as deviance residual with site-level count data. The p-values are then used as a scoring metric in `{gsm}` to flag possible outliers using the thresholds discussed below.

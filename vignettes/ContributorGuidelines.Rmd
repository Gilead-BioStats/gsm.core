...
Code developers for the `{gsm}` packates use the [tidyverse style guide](https://style.tidyverse.org/) with minimal modifications. The code below is run to standardize styling before each release:
...
-   Qualification Report (`qualification-report`):
    -   Builds the qualification vignette as an attached artifact to a release tagged with he Pull Request name via GHA dispatch to `{gsm.qc}`. This should be reviewed by the Pull Request Owner for completeness and correctness to ensure that the artifact added to the release is correct.
